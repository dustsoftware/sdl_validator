...
RSpec.describe "SDLValidator::Validator"  do
  ...
  subject(:validator){ SDLValidator::Validator.new(sip)}
  ...
  describe "missing fields" do
    it "raise error if required fields are missing" do
      expect(SDLValidator::Validator.new().valid?).to eq false
    end
  end
  ...
  describe "attribute with -" do
    it "is valid if specified" do
      SDLValidator::Validator.new(sip.merge({'discussions-to':'something'}))
    end
 
    it "not valid if not specified" do
      expect{
        SDLValidator::Validator.new(sip.merge({'how-to':'something'}))
  ...
  describe "attribute with _" do
    it "not valid" do
      expect{
        SDLValidator::Validator.new(sip.merge({'discussions_to':'something'}))
